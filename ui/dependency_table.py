...
class DependencyTable(QTableWidget):
    ...
    def _setup_table(self) -> None:
        ...
        self.setColumnWidth(COL_CURRENT, 110)
    ...
    def update_row(self, dep: DependencyInfo) -> None:
        ...
        if row is None:
            return
        cur_item = self.item(row, COL_CURRENT)
        if cur_item:
            cur_item.setText(dep.raw_constraint)
    ...
    def _add_row(self, dep: DependencyInfo) -> None:
        ...
        cur_item = QTableWidgetItem(dep.raw_constraint)
        cur_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        cur_item.setFont(mono)
        self.setItem(row, COL_CURRENT, cur_item)
    ...
    def _refresh_package_cell(self, row: int, dep: DependencyInfo) -> None:
        ...
        if dep.repo_url:
            btn = QPushButton(_repo_label(dep.repo_url))
            btn.setObjectName("pkgLinkBtn")
            btn.setFixedHeight(22)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.setToolTip(dep.repo_url)
            url = dep.repo_url          # capture by value
    ...
    def _refresh_version_cells(self, row: int, dep: DependencyInfo) -> None:
        ...
        versions = {
            ...
            COL_MINOR: (dep.latest_minor, dep.minor_age, "minor"),
    ...
    def _apply_filter_to_row(self, row: int, dep: DependencyInfo) -> None:
        ...
        if self._filter_group and dep.group != self._filter_group:
            hidden = True
        if self._hide_uptodate and not dep.has_any_update and dep.fetch_status == "done":
            hidden = True
