...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._fetch_total = 0
        self._prev_page: int = 0   # page to return to when leaving settings
    ...
    def _run_npm_install(self) -> None:
        ...
        NpmInstallDialog(project_dir, parent=self)
    ...
    def _on_settings_changed(self, settings: AppSettings) -> None:
        old_min_age_days = self._settings.min_age_days
        self._settings = settings
        self._apply_palette()
        self._apply_stylesheet()
        self._table.set_dark(settings.dark_mode)
        if self._file_path:
            # Bypass cache only when min_age_days changed — cached results from
            # the old filter would include versions that are now excluded (or vice versa).
            self._start_fetch(bypass_cache=old_min_age_days != settings.min_age_days)
